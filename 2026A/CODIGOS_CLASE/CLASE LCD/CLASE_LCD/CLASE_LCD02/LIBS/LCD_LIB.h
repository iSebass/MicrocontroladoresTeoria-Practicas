...
void lcd_init(void);
void lcd_write(uint8_t letra); // ESCRIRBIR UNA LETRA
void lcd_command(uint8_t cmd); // ESCRIBIR UN COMANDO

void lcd_puts(char *str);      // ESCRIBIR TEXTO
void lcd_set_cursor(uint8_t row, uint8_t col); // POSICIONARME EN LA LCD
void lcd_printf(char *str, ...); // TEXTO CON FORMATO

void lcd_clear(void);  // LIMPIAR LA LCD
void lcd_return_home(void); // REGRESAR EL CURSOR A CASA
void lcd_on(void); // ENCENDER LA LCD
void lcd_off(void); // APAGAR LA LCD

void lcd_enable_blink(void);  // ENCENDER EL PARPADEO
void lcd_disable_blink(void); // APAGAR EL PARPADEO

void lcd_enable_cursor(void);  // ENCENDER CURSOR
void lcd_disable_cursor(void); // APAGAR EL CURSOR

void lcd_scroll_left(void);   // MOVER EL TEXTO A LA IZQUIERDA
void lcd_scroll_right(void);  // MOVER EL TEXTO A LA DERECHA

void lcd_custom_char(uint8_t mem, uint8_t *charmap); //CREAR MI PROPIO CARACTER
